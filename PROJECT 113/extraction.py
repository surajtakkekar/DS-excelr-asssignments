...
class ResumeParser:
            ...
            if i.lower() == 'skills':
                ...
                while (j < len(tokens) or x != '\n'):
                    if (nlp(x)[0].pos_ in ['NOUN', 'VERB'] or x.lower() in temp.lower()):
                        ...
                        if x == '\n':
                            break
                        if (nlp(x)[0].pos_ == 'NOUN' or nlp(x)[0].pos_ == 'VERB') and j < len(tokens) and \
                                nlp(tokens[j])[0].pos_ not in ['NOUN', 'SPACE', 'PUNCT']:
                            ans = ans + ','
                    j += 1
                ...
                while j < len(tokens) or x != '\n':
                    if (nlp(x)[0].pos_ in ['NOUN', 'VERB'] or x.lower() in temp.lower()):
                        ...
                        if x == '\n':
                            break
                        if (nlp(x)[0].pos_ == 'NOUN' or nlp(x)[0].pos_ == 'VERB') and j < len(tokens) and \
                                nlp(tokens[j + 1])[0].pos_ not in ['NOUN', 'SPACE', 'PUNCT']:
                            ans = ans + ', '
                    j += 1
                    if j >= len(tokens) or x == '\n':
                        break
    ...
    def extract_phone_number(self, resume_text):
        ...
        '''
        try:
            return re.search(re.compile(r'(\+91)?(-)?\s*?(91)?\s*?(\d{3})-?\s*?(\d{3})-?\s*?(\d{4})'),
                             resume_text).group().strip()
        except:
            return ""
